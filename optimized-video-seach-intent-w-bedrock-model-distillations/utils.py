...
def delete_role_and_attached_policies(role_name):
    try:
        ...
        print(f"Unexpected error: {str(e)}")
        return False
def add_evaluation_permissions(role_name, account_id):
    """Update IAM role to allow Bedrock evaluation jobs and model invocation."""
    iam = boto3.client("iam")

    # Update trust policy to allow evaluation jobs
        ...
        "Statement": [
            {
                ...
                "Condition": {
                    ...
                    "ArnLike": {
                        "aws:SourceArn": [
                            f"arn:aws:bedrock:*:{account_id}:model-customization-job/*",
                            f"arn:aws:bedrock:*:{account_id}:evaluation-job/*",
                        ]
                    },
                },
            }
    ...
    print(f"Updated trust policy for '{role_name}' to allow evaluation jobs")

    # Add InvokeModel permissions for all model types
    eval_policy_name = f"bedrock_eval_invoke_policy_{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    eval_invoke_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
                ...
                "Resource": [
                    f"arn:aws:bedrock:*:{account_id}:inference-profile/*",
                    f"arn:aws:bedrock:*:{account_id}:custom-model-deployment/*",
                    "arn:aws:bedrock:*::foundation-model/*",
                ],
            }
        ],
    ...
    policy_resp = iam.create_policy(
        PolicyName=eval_policy_name,
        PolicyDocument=json.dumps(eval_invoke_policy),
        Description="Allow Bedrock eval jobs to invoke all model types",
    )
    iam.attach_role_policy(
        RoleName=role_name,
        PolicyArn=policy_resp["Policy"]["Arn"],
    )
    print(f"Attached eval invoke policy: {eval_policy_name}")
